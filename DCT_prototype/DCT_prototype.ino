...
#include "src/DCTHSK_lib/support_functions.h"
...
PacketSerial upStream1;  // Pot A SHOULDNT BE PACKET SERIAL!
PacketSerial upStream2;  // Pot B
...
//  upStream1.setPacketHandler(&checkHdr);
  ...
  // functions begin SPI connections
  Initialize_TM4C123();
  configure_channels();      
...
//    whatToDoIfThermistors(buffer);
  ...
  case eThermistorsTest:{
    retval = 4; // length of the thermistor channels to send back?
    float res = return_resistance(CHIP_SELECT, 10);
